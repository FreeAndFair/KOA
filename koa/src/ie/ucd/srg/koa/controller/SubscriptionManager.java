/** -----------------------------------------------------------------------
  *
  *   ie.ucd.srg.koa.controller.SubscriptionManager.java
  *
  * -----------------------------------------------------------------------
  * 
  ...
  * -----------------------------------------------------------------------
  */
package ie.ucd.srg.koa.controller;
...
import ie.ucd.srg.koa.controller.subscription.ESBClientSubscription;
import ie.ucd.srg.koa.controller.subscription.KRClientSubscription;
...
public class SubscriptionManager
{
	...
	private void checkConfiguredESBSubscriptions() throws KOAException
	{
		...
		{
			sComponent = (String) enumer.nextElement();
	...
	private void checkConfiguredKRSubscriptions() throws KOAException
	{
		...
		while (enumer.hasMoreElements())
		{
			sComponent = (String) enumer.nextElement();
			/* if it is not subscribed in the controller, subscribe it */
			if (SubscriptionManager.getClientSubscription(sComponent) == null)
			{
				/* get the parameters from the properties */
				String sJNDIName =
					JNDIProperties.getProperty(
