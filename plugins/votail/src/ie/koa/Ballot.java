...
/**
  ...
  protected /*@ spec_public non_null@*/ long[] preferenceList = new long [0];
  ...
  protected /*@ spec_public non_null @*/ long[] candidateIDAtCount = 
    new long [MAXIMUM_ROUNDS_OF_COUNTING];
  ...
  public /*@ pure @*/ long getPreferenceAtCount(int countNumber) {
    if (0 <= countNumber && countNumber <= countNumberAtLastTransfer) {
      long number = candidateIDAtCount[countNumber];
      return number;
    } else {
      return 0;
    }
  ...
  public void load(/*@ non_null @*/ long[] candidateIDList, int listSize, long uniqueID){
    // numberOfPreferences == 0;
    // countNumberAtLastTransfer == 0;
    // positionInList == 0;
    // candidateID == NONTRANSFERABLE; 
    ...
    ballotID = getUniqueNumber();
    preferenceList = new long [listSize];
  ...
  private /*@ helper @*/ void shiftPreferenceList() {
	  if( 0 < positionInList && positionInList < numberOfPreferences-1){
		long temp = preferenceList[positionInList+1];
  ...
  */    
  ...
  // @TODO specify randomness in terms of secure information flow:
  // i.e. the order of ballots does not depend on the order in which the ballots
  // were cast or on any of the preferences, equivalent to a perfect hash code.
  // @see System.currentTimeMills();
  private static volatile long uniqueID = 0;
  // @design Patrick and Joe - 
  /*@
    @ ensures \result >= 0;
    @ ensures (* note this really isn't random because ... *);
    @*/	
  private synchronized/*@ helper @*/long getUniqueNumber() {
		long num = (System.currentTimeMillis() * uniqueID++) % MAX_POSSIBLE_BALLOTS;
		if(num>0){
			return num;
		}
		return 0;
