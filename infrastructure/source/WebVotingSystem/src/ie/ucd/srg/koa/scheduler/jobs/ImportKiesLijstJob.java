/** -----------------------------------------------------------------------
  *
  *   ie.ucd.srg.koa.scheduler.job.ImportKiesLijstJob.java
  *
  * -----------------------------------------------------------------------
  * 
...
import ie.ucd.srg.koa.databeheer.ejb.session.KieslijstAdmin;
import ie.ucd.srg.koa.databeheer.ejb.session.KieslijstAdminHome;
 ...
 * @author KuijerM
 */
public class ImportKiesLijstJob extends AbstractJob
{
	/**
	 * Constructor for the ImportKiesLijstJob
	 */
	public ImportKiesLijstJob()
	{
		KOALogHelper.log(
			KOALogHelper.TRACE,
			"[ImportKiesLijstJob] Constructor...");
	}
	/**
	 * Executes the job
	 * 
	...
	public boolean execute() throws EPlatformException
	{
		KOALogHelper.log(
			KOALogHelper.TRACE,
			"[ImportKiesLijstJob.execute] Start executing the importing...");
		...
		try
		{
			...
			KOALogHelper.log(
				KOALogHelper.TRACE,
				"[ImportKiesLijstJob.execute] got temp data id "
					+ context.getProperty(SchedulerConstants.TEMP_DATA_ID_KEY));
		}
		...
		try
		{
			...
			KOALogHelper.log(
				LogHelper.INFO,
				"[ImportKiesLijstJob] start process import kieslijst");
			bean.processImport(iTempDataID);
			KOALogHelper.log(
				LogHelper.INFO,
				"[ImportKiesLijstJob] finished process import kieslijst");
		}
	...
	public boolean init() throws EPlatformException
	{
		KOALogHelper.log(
			KOALogHelper.TRACE,
			"[ImportKiesLijstJob.init] initialize...");
		return true;
	}
}