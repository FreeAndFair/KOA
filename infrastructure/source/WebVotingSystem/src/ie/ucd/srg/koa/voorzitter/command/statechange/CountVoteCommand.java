/** -----------------------------------------------------------------------
  *
  *   ie.ucd.srg.koa.command.PrepareVoteCountCommand.java
  *
  * -----------------------------------------------------------------------
  * 
 ...
 * 
 */
public class CountVoteCommand extends AbstractStateChangeCommand
{
	/**
	 * The execute method on the CountVote command.
	...
	{
		LogHelper.log(LogHelper.INFO, "[CountVoteCommand] execute ");
		try
		{
			/* init the variabeles */
			...
			{
				/* change state to preparevote count */
				g_crCallResult = xController.countVote();
			}
		}
		catch (NamingException ne)
		{
			...
			KOALogHelper.logErrorCode(
				"CountVoteCommand.execute",
				ErrorConstants.ERR_NAMING,
				params,
				ne);
			throw new KOAException(
				ErrorConstants.COMMAND_PREPARECOUNTVOTE_EXEC,
				ne);
		}
	...
	{
		LogHelper.trace(LogHelper.TRACE, "[CountVoteCommand] init");
		/* get the pincodes */
